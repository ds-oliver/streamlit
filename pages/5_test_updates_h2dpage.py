...
def get_top_players(team, player_df, stat, top=5):
    ...
    if 'team' not in player_df.columns:
        ...
        print('team column in player_df')
        top_players_matchup = player_df.groupby(['matchup_merge_key', 'team']).agg({stat: 'sum'}).reset_index()
        top_players_matchup = top_players_matchup[top_players_matchup['team'] == team].sort_values(by=stat, ascending=False).head(top)
